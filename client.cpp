...
class State {
  ...
  private:
    ...
    NOINLINE void found_solution() COLD;
    NOINLINE void found_tie(uint nr_cols) COLD;
...
void State::found_solution() {
    max_cols_ = max_col_;

    uint8_t buffer[1+(MAX_ROWS+MAX_COLS-1+7)/8];
    buffer[0] = max_col_;
    uint bytes = 1 + (rows+max_col_-1+7)/8;
    Sum const* s = &side[0];
    ...
    id_out << "rows=" << rows << ",cols=" << max_col_ << " (" << elapsed() << " s)\n";
    for (auto r=0U; r<rows;r++) {
        for (auto c=0U; c<max_col_;c++) {
...
void State::found_tie(uint nr_cols) {
    max_col_ = max_cols_;
    auto& info = col_info[current_col_rev_];
    if (nr_cols == max_col_ && nr_cols > info.min) {
        info.min = nr_cols;
...
uint State::extend(uint col, bool zero, bool one) {
    ...
    if (UNLIKELY(col1 >= max_col_)) {
        max_col_ = col1;
        lock_guard<mutex> lock{mutex_info};
        if (UNLIKELY(max_col_ > max_cols_))
            found_solution();
        else
            found_tie(col1);
    }
